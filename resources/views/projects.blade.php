...
<div class="min-h-screen pt-20 bg-gradient-to-b from-primary-50 to-white">
    ...
    <a href="{{ url('/projects') }}"
       class="px-6 py-2 rounded-full font-medium text-sm transition-all duration-300 
              {{ !$currentCategory ? 'bg-gradient-to-r from-primary-600 to-primary-700 text-white shadow-lg scale-105' 
        ...
        <a href="{{ url('/projects?category=' . $category->id) }}"
           class="px-6 py-2 rounded-full font-medium text-sm transition-all duration-300 
                  {{ $currentCategory == $category->id 
                     ...
                     : 'bg-white text-secondary-700 hover:bg-primary-50 shadow border border-secondary-200 hover:border-primary-300' }}"
           data-key-uz="{{ $category->name_uz }}"
           data-key-en="{{ $category->name_en }}">
            {{ $category->name_en }}
        </a>
    @endforeach
</div>
                    ...
                    @foreach ($projects as $project)
                        <div class="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border border-secondary-200">
                            <div class="relative h-56 overflow-hidden">
                                <img src="{{ $project->image ? asset('storage/'.$project->image) : 'https://via.placeholder.com/800x400?text=No+Image' }}"
