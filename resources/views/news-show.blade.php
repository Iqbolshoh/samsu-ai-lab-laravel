    ...
    <div class="min-h-screen pt-20">
        ...
        <article class="py-12 bg-white">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <figure class="mb-10">
                    <div class="relative rounded-2xl overflow-hidden shadow-2xl border border-secondary-200">
                        <img id="articleImage"
                            ...
                            alt="{{ $news->title_en }}" class="w-full h-auto object-cover">
                    </div>
                </figure>
                ...
                @if($news->video)
                    @php
                        preg_match('/(youtu\.be\/|embed\/|v=)([A-Za-z0-9_-]+)/', $news->video, $m);
                        $id = $m[2] ?? null;
                    @endphp

                    @if($id)
                        <div class="mb-10 rounded-2xl overflow-hidden shadow-xl">
                            <iframe src="https://www.youtube.com/embed/{{ $id }}?modestbranding=1&rel=0" class="w-full aspect-video"
                                allowfullscreen>
                            </iframe>
                        </div>
                    @endif
